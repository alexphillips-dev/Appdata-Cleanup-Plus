...
behaviorSmokeAssertSame(5, isset($diagnosticsBundle["schemaVersion"]) ? (int)$diagnosticsBundle["schemaVersion"] : 0, "Diagnostics bundle should declare the structured troubleshooting schema version.");
behaviorSmokeAssertTrue(isset($diagnosticsBundle["collection"]["sections"]["quarantineRegistry"]["status"]), "Every state collector should declare completeness.");
$failedMetadata = array();
$failedData = appdataCleanupPlusDiagnosticsCollect(function() { throw new RuntimeException("private-name password=secret /mnt/user/Private/path"); }, $failedMetadata);
behaviorSmokeAssertSame(array(), $failedData, "A failed collector should not discard other sections.");
behaviorSmokeAssertSame("failed", $failedMetadata["status"], "Failed collectors must be explicit.");
behaviorSmokeAssertNotContains("private-name", json_encode($failedMetadata), "Collector errors must omit exception messages.");
$partialMetadata = array();
appdataCleanupPlusDiagnosticsCollect(function() { return array("status"=>"partial", "includedCount"=>50,"omittedCount"=>10); }, $partialMetadata);
behaviorSmokeAssertSame(10, $partialMetadata["omittedCount"], "Known omitted counts should be preserved.");
behaviorSmokeAssertSame("unavailable", appdataCleanupPlusDiagnosticsCurrentSnapshot("")["status"], "An absent current scan is explicitly unavailable.");
$diagnosticSnapshot = writeAppdataCleanupPlusSnapshot(array("example"=>array("path"=>"/synthetic/private")));
behaviorSmokeAssertSame("valid", appdataCleanupPlusDiagnosticsCurrentSnapshot($diagnosticSnapshot["token"])["status"], "Current snapshot inspection should recognize a valid scan.");
$diagnosticSnapshot["expiresAt"] = date("c", time()-60);
$diagnosticSnapshotFile = appdataCleanupPlusSnapshotFile($diagnosticSnapshot["token"]);
writeAppdataCleanupPlusJsonFile($diagnosticSnapshotFile, $diagnosticSnapshot);
$snapshotHash = hash_file("sha256", $diagnosticSnapshotFile);
$expiredDiagnostic = appdataCleanupPlusDiagnosticsCurrentSnapshot($diagnosticSnapshot["token"]);
behaviorSmokeAssertSame("expired", $expiredDiagnostic["status"], "Expired snapshots should be reported.");
behaviorSmokeAssertTrue(is_file($diagnosticSnapshotFile), "Diagnostics must not prune expired snapshots.");
behaviorSmokeAssertSame($snapshotHash, hash_file("sha256", $diagnosticSnapshotFile), "Snapshot inspection must not mutate state.");
behaviorSmokeAssertNotContains($diagnosticSnapshot["token"], json_encode($expiredDiagnostic), "Snapshot tokens must never enter diagnostics.");
behaviorSmokeAssertNotContains("private", json_encode($expiredDiagnostic), "Snapshot candidates must not enter freshness metadata.");
