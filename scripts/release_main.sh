...
manifest and CA XML, and print the exact cache-busting install URL.
...
verify_remote_release_metadata() {
    ...
    local version="${3:-}"
    local commit_hash="${4:-}"
    local manifest_url="https://raw.githubusercontent.com/${remote_slug}/refs/heads/${branch_name}/plugins/appdata.cleanup.plus.plg"
    local xml_url="https://raw.githubusercontent.com/${remote_slug}/refs/heads/${branch_name}/appdata.cleanup.plus.xml"
    ...
    if [[ "${manifest_content}" != *"<!ENTITY version \"${version}\">"* ]]; then
    ...
    if [[ "${xml_content}" != *"<PluginURL>https://raw.githubusercontent.com/${remote_slug}/refs/heads/${branch_name}/plugins/appdata.cleanup.plus.plg</PluginURL>"* ]]; then
...
if [ "${DRY_RUN}" = true ]; then
    ...
    echo "  6. Verify the live raw manifest and CA XML on ${MAIN_BRANCH}"
...
git add "${PLG_FILE}" "${XML_FILE}" "${ARCHIVE_FILE}"
...
CACHE_BUSTING_URL="https://raw.githubusercontent.com/${REMOTE_SLUG}/${HEAD_COMMIT}/plugins/appdata.cleanup.plus.plg"
