...
grep -Fq 'appdataCleanupPlusDiagnosticsFixtureRootSummary()' <<<"${server_bundle}" || fail "Fixture root diagnostics must use a path-free schema summary."
grep -Fq 'Fixture diagnostics must exclude arbitrary status fields and response bodies' "${ROOT_DIR}/tests/maintenance_ui.cjs" || fail "Fixture failure privacy coverage is missing."
grep -Fq 'Fixture root diagnostics must contain only fixed codes and counts' "${ROOT_DIR}/tests/behavior_smoke.php" || fail "Fixture root privacy coverage is missing."
