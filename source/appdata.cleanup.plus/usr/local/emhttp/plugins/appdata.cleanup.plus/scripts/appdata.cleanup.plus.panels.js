(function(window, document, $) {
  ...
  ACP.renderModeStrip = function(context) {
    ...
    var buttonLabel = state.quarantine && state.quarantine.open
      ? ACP.t(strings, "quarantineManagerCloseLabel", "Close manager")
  ...
  ACP.renderQuarantinePanel = function(context) {
    ...
    var html = [
      ...
      '<div class="acp-utility-head">',
      '<div class="acp-utility-copy">',
      '<div class="acp-utility-title">' + ACP.escapeHtml(ACP.t(strings, "quarantineManagerTitle", "Quarantine manager")) + "</div>",
      '<div class="acp-utility-subtitle">' + ACP.escapeHtml(summary.count ? (summary.count + " " + (summary.count === 1 ? ACP.t(strings, "quarantineCountSingular", "quarantined folder") : ACP.t(strings, "quarantineCountPlural", "quarantined folders")) + " tracked" + (summary.sizeLabel ? " | " + summary.sizeLabel : "")) : ACP.t(strings, "quarantineSummaryEmpty", "No quarantined folders are tracked right now.")) + "</div>",
      "</div>",
      '<div class="acp-utility-actions">',
      ...
      '<button type="button" class="acp-button acp-button-secondary" data-action="toggle-quarantine">' + ACP.escapeHtml(isOpen ? ACP.t(strings, "quarantineManagerCloseLabel", "Close manager") : ACP.t(strings, "quarantineManagerOpenLabel", "Open manager")) + "</button>",
      "</div>",
      "</div>"
    ];
    ...
    if (isOpen) {
      ...
      if (quarantine.loading) {
        ...
        html.push('<div class="acp-quarantine-list">');
        $.each(entries, function(_, entry) {
          html.push('<article class="acp-quarantine-entry">');
          html.push('<div class="acp-quarantine-head">');
          html.push('<div class="acp-quarantine-title-wrap">');
          html.push('<div class="acp-quarantine-title">' + ACP.escapeHtml(entry.name || entry.sourcePath || "") + "</div>");
          html.push('<div class="acp-quarantine-meta">' + ACP.escapeHtml(entry.quarantinedAtLabel || "") + (entry.quarantinedAgeLabel ? " | " + ACP.escapeHtml(entry.quarantinedAgeLabel) : "") + (entry.sizeLabel ? " | " + ACP.escapeHtml(entry.sizeLabel) : "") + "</div>");
          html.push("</div>");
          html.push('<div class="acp-quarantine-actions">');
          html.push('<button type="button" class="acp-button acp-button-secondary acp-row-action" data-entry-action="restore" data-entry-id="' + ACP.escapeHtml(entry.id || "") + '">' + ACP.escapeHtml(ACP.t(strings, "quarantineRestoreActionLabel", "Restore")) + "</button>");
          html.push('<button type="button" class="acp-button acp-button-secondary acp-row-action" data-entry-action="purge" data-entry-id="' + ACP.escapeHtml(entry.id || "") + '">' + ACP.escapeHtml(ACP.t(strings, "quarantinePurgeActionLabel", "Purge")) + "</button>");
          html.push("</div>");
          html.push("</div>");
          html.push('<div class="acp-quarantine-paths">');
          html.push('<div class="acp-quarantine-path-group"><span class="acp-modal-result-label">' + ACP.escapeHtml(ACP.t(strings, "quarantineSourceLabel", "Original")) + '</span><code class="acp-modal-path">' + ACP.escapeHtml(entry.sourcePath || "") + "</code></div>");
          html.push('<div class="acp-quarantine-path-group"><span class="acp-modal-result-label">' + ACP.escapeHtml(ACP.t(strings, "quarantineLocationLabel", "Quarantine path")) + '</span><code class="acp-modal-path acp-modal-path-secondary">' + ACP.escapeHtml(entry.destination || "") + "</code></div>");
          html.push("</div>");
          html.push("</article>");
        });
        html.push("</div>");
    ...
    html.push("</section>");
    els.$quarantinePanel.html(html.join(""));
  };
